...
export function reportAndRecordFetch(fetchReport, sources, env) {
  return async (results) => {
    ...
    const failures = fetchReport.results.filter((r) => !r.count);

    const successes = fetchReport.results.filter((r) => r.count > 0);

    failures.map((r) => {
      log.debug(r);
    });
    log.info(
      `Dry run finished with ${successes.length} successes and ${
        failures.length
      } failures in ${
        (fetchReport.timeEnded - fetchReport.timeStarted) / 1000
      } seconds`
    );
