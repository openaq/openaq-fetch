 ...
 */
'use strict';
import {removeUnwantedParameters, unifyParameters} from '../lib/utils.js';
...
export const name = 'netherlands2';
...
export async function fetchData (source, cb) {
  ...
  let totalStations = await new Promise((resolve, reject) => {
  ...
  for (let i = 0; i < totalStations; i++) {
    let pageStations = await new Promise((resolve, reject) => {
...
let formatData = function (results) {
  ...
  data.forEach(item => {
    ...
    item.data.forEach(data => {
      ...
      let dateMoment = moment(data.timestamp_measured).add(1, 'hours');
