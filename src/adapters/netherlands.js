 ...
 * and returning data for the Netherlands data source.
 */
...
export function fetchData(source, cb) {
  ...
  client({ url: finalURL, responseType: 'text' })
    .then((body) => {
      ...
      let recentDate = _.last(fileList).date.getTime();
      let recentFiles = _.filter(fileList, function (f) {
      ...
      async.parallel(tasks, function (err, results) {
        ...
        result.measurements = removeUnwantedParameters(
          result.measurements
        );
    ...
    .catch((err) => {
      cb({ message: 'Failure to load data url.', error: err });
    });
}

// Parses an Apache directory listing and returns an array with the files
// in the root
const listApachetree = function (data, url) {
  let $ = load(data);

  const parseDate = function (ds) {
    ds = ds.trim();
    let date = DateTime.fromFormat(ds, 'yyyy-MM-dd HH:mm', {
...
const formatData = function (name, data) {
  let $ = load(data, { xmlMode: true });

  // Determine what parameter is measured from the file-name
  let regExp = new RegExp('[0-9]+-(.+).xml');
  let p = name.match(regExp)[1].replace('.', '').toLowerCase();

  const parseDate = function (string) {
    string = string.trim();
    let date = DateTime.fromFormat(string, 'yyyyMMddHHmmss', {
      zone: 'Europe/Amsterdam',
    });
  ...
  const getStationId = function (string) {
    ...
    let li = string.length === 3 ? 'NL10' + string : string;
    return li;
  };
  ...
  const getCity = function (string) {
    let splitLocation = string.split('-');
    let cityName = splitLocation[0].replace('PNH ', '');
    return cityName;
  };

  // RIVM has to be attributed first. If another organization is
  // mentioned, pass in it in the second place.
  const getAttribution = function (string) {
    let attribution = [
  ...
  const getPeriod = function (string) {
  ...
  $('ROW').each(function (i, elem) {
    let stationID = getStationId($('STAT_NUMMER', this).text());

    let m = {
      date: parseDate($('MWAA_EINDDATUMTIJD', this).text()),
      parameter: p,
      location: $('STAT_NAAM', this).text(),
      value: parseFloat($('MWAA_WAARDE', this).text()),
      unit: 'µg/m³',
      stationId: stationID,
      city: getCity($('STAT_NAAM', this).text()),
      attribution: getAttribution(
        $('OPST_OPDR_ORGA_CODE', this).text()
      ),
