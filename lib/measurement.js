 ...
 */
export function createFetchObject (input, source, dryRun = false) {
  let fetchEnded = NaN;
  let count = 0;
  const failures = {};
  ...
  return stream
    .map(async (error) => {
      if (error instanceof FetchError) {
        const cause = error.type.toString();
  ...
  const out = stream.map(
    async measurement => {
      ...
      if (v.errors.length === 0) {
        ...
        return new FetchError(MEASUREMENT_ERROR, source, {measurement, errors: v.errors}, 0);
      }
    }
  );
