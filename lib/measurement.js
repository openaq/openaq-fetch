 ...
 */
export function createFetchObject (input, source, dryRun, verbose) {
  let fetchEnded = NaN;
  let count = 0;
  const failures = {};

  const stream = input
    .use(handleMeasurementErrors, failures, verbose)
    .do(() => count++)
  ;
 ...
 * @param {JetLog} errLog
 */
export function handleMeasurementErrors (stream, failures, verbose) {
  return stream
    .map(async (error) => {
 ...
 */
export async function getMeasurementsFromSource (source, {dryrun, verbose}) {
  ...
  return createFetchObject(out, source, dryrun, verbose);
}
