...
async function saveResultsToS3 (stream, s3, bucketName, key) {
  return stream
    .use(streamDataToS3, s3, bucketName, key)
  ;
}

async function streamDataToS3 (stream, s3, bucketName, key) {
  await new Promise((resolve, reject) => {
    ...
    upload.maxPartSize(1048576);
    upload.concurrentParts(5);

    log.debug(`Uploading data to s3 as ${bucketName}/${key}`);
 ...
 */
export function streamMeasurementsToDBAndStorage (stream, {doSaveToS3, doSaveToDB, dryrun}, bucketName) {
