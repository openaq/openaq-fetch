 ...
 * @return {array} An array pruned of invalid measurement objects, may be empty
 */
exports.pruneMeasurements = function (measurements) {
  ...
  return _.filter(measurements, function (m) {
    ...
    if (m.attribution) {
      ...
      if (!m.attribution instanceof Array) {
        return false;
      }

      // Make sure it doesn't have any empty elements
      let valid = true;
      m.attribution.forEach((a) => {
        if (a.name === '' || a.url === '') {
          valid = false;
          return;
        }
